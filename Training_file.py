...
path = 'C:\\Users\\esha kolte\\Desktop\\Attendence_Using_AI\\background.jpg'
...
def Capture_images(): 
    Id=(txt.get())
    name=(txt2.get())
    if(check_number(Id) and name.isalpha()):
...
def train_images():
    ...
    harcascadePath = "haarcascade_frontalface_default.xml"
    detector =cv2.CascadeClassifier(harcascadePath)
    faces,Id = getImages_Labels("TrainingImage")
    recognizer.train(faces, np.array(Id))
    recognizer.save("TrainingFile\Trainner.yml")
    res = "Dataset Trained Successfully"
    message.configure(text= res)
    playsound('Dataset-Trained-Successfully.mp3')


def getImages_Labels(path):
...
def track_images():
    ...
    recognizer.read("C:\\Users\\esha kolte\\Desktop\\Attendence_Using_AI\\TrainingFile\\Trainner.yml")
    harcascadePath = "C:\\Users\\esha kolte\\Desktop\\Attendence_Using_AI\\haarcascade_frontalface_default.xml"
    faceCascade = cv2.CascadeClassifier(harcascadePath);    
    df=pd.read_csv("C:\\Users\\esha kolte\\Desktop\\Attendence_Using_AI\\Details\\StudentDetails.csv")
    cam = cv2.VideoCapture(0)
    ...
    while True:
        ...
        for(x,y,w,h) in faces:
            ...
            if(conf < 50):
                ...
                tt=str(Id)  
            if(conf > 75):
                noOfFile=len(os.listdir("ImagesClicked"))+1
                cv2.imwrite("C:\\Users\\esha kolte\\Desktop\\Attendence_Using_AI\\ImagesClicked\\Image"+str(noOfFile) + ".jpg", im[y:y+h,x:x+w])            
...
clearButton2 = tk.Button(window, text="-", command=clear1, fg="white", bg="red", width=3, height=1, activebackground = "Red" ,font=('times', 10, ' bold '))
clearButton2.place(x=400, y=235)    
takeImg = tk.Button(window, text="Register", command=Capture_images, fg="white", bg="grey", width=10, height=1, activebackground = "aqua" ,font=('times', 15, ' bold '))
takeImg.place(x=100, y=280)
trainImg = tk.Button(window, text="Trained", command=train_images, fg="white", bg="blue", width=10, height=1, activebackground = "gold" ,font=('times', 15, ' bold '))
trainImg.place(x=260, y=280)
trackImg = tk.Button(window, text="Mark Attendance", command=track_images, fg="white", bg="green", width=14, height=1, activebackground = "lime" ,font=('times', 15, ' bold '))
